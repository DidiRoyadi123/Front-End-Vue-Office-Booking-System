             ...
             <!-- Modal delete -->
              <b-modal hide-footer hide-header centered :id="'modalDelete' + user.id" >
                ...
                <div class="col-12 text-center">
                  <b-button class="mt-3" variant="success" @click="$bvModal.hide('modalDelete' + user.id)">Batal</b-button>
                  <b-button class="mt-3" variant="danger" @click="deleteUser(user.id), $bvModal.hide('modalDelete'+ user.id)">
                    Ok
                  </b-button>
                </div>
              </b-modal>
...
export default {
  ...
  methods: {
    ...
    deleteUser(id) {
      axios
        .delete('https://officebooking-app-pn6n3.ondigitalocean.app/admin/user/' + id)
        .then(response => {
          this.fetchUsers()
